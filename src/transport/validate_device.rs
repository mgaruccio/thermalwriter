...
//
// Passive `thermalwriter validate-device` preflight: descriptor/sysfs inventory only.
    ...
    F: SysfsAccess,
{
        ...
        args.passive,
        "active validate-device workflow is not implemented yet; pass --passive for descriptor-only preflight"
    );
...
struct PassivePreflightContext<'a, I, H, F> {
    vid: u16,
    pid: u16,
...
fn run_passive_preflight<I, H, F>(ctx: PassivePreflightContext<'_, I, H, F>) -> PassiveOutcome
where
    I: UsbInventory,
    H: HidrawInventory,
    F: SysfsAccess,
...
mod tests {
    ...
    #[test]
    fn non_passive_mode_is_rejected() {
        let temp = tempfile::tempdir().unwrap();
        let error = run_validate_device_with(
            ValidateDeviceArgs {
                device: "0416:5302".to_string(),
